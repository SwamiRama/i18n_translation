...
namespace :translate do
  ...
  task merge_keys: :environment do
    ...
    fail('Can only merge in translations in single locale') if new_translations.keys.size > 1
  ...
  task google: :environment do
    fail 'Please specify FROM and TO locales as environment variables' if ENV['FROM'].blank? || ENV['TO'].blank?
